...
contract connect {
    ...
    function makeMove(uint8 column) public {
        PlayerState memory state = playerStates[msg.sender];
        require(state.boardState[5][column] == 0, "no empty spaces");
        uint8 row = get_next_open_row(state.boardState, column);
        drop_piece(msg.sender, row, column, 1);
        if (winning_move(playerStates[msg.sender].boardState, 1)) {
            ...
            row = get_next_open_row(state.boardState, col);
            drop_piece(msg.sender, row, col, 2);
